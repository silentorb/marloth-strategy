...
The ASCII console prototype runs an open-ended session across switchable modal screens: the player advances production by one selected **step resolution** (from `tick` through the configured calendar units), adjusts that resolution with `-` / `+`, switches between the **workflow** and **actors** screens with `w` / `a`, sees a clear-and-redraw bordered panel report after each advance, and can quit at any prompt. There is no win or loss condition yet.

## When to read this

- Changing player-facing console commands, modal screens, turn pacing at the session layer, or what is shown after a tick
...
- Modal screens:
  - Each session (and each **new game**) starts on the **workflow** screen (node state + flow graph).
  - **`w`** / **`W`** selects the workflow screen; **`a`** / **`A`** selects the actors screen (roster properties and assignments). Switching redraws without advancing time.
  - Enter / `-` / `+` / `n` / `q` work on every screen.
- At the prompt (interactive terminal: single keypress, no extra Enter after `q` or `=`):
  ...
  - **`w`** / **`W`** shows the workflow screen.
  - **`a`** / **`A`** shows the actors screen.
  - **`n`** / **`N`** starts a new game with the session's configured scenario and seed, returning to the tick-0 **workflow** screen while keeping the selected step resolution.
  ...
  - When stdin is redirected (non-interactive), the same rules apply in line mode (empty line = Enter, `-` / `+` / `=` lines = resolution controls, `w` / `a` lines = screen switches, `n` line = new game, `q` line = quit).
...
- After each advance, the console **clears** and shows the **current** modal screen:
  - **Workflow:** a top status strip (`Tick N`, calendar positions such as `month 1, week 1/4, day 1/7`, `screen: workflow`, `scenario: {preset-or-random} seed {N}`, `actors:`), a left column of per-node subpanels (port stocks and cumulative **cycles** on the left of each subpanel; signed net change since tick 0 in a middle **Δ** column (money counts everything that has flowed through a port, so sale income reads `+` and paid wages read `-` even though those ports hold no stock); preferred actor assignments and weights on the right), and a right column with a laid-out flow graph of nodes and edges. Leaf values that changed this advance are annotated as `previous → current` (Unicode right arrow), comparing the state before the whole advance to the state after. Unchanged leaves show the current value only.
  - **Actors:** the same status strip with `screen: actors`, then one full-width subpanel per actor (properties on the left: capacity, wage, stats; preferred assignments on the right as `nodeId weight`).
- Before the first prompt, the console shows the workflow screen for **tick 0** (no change arrows) so the player has a baseline.
