...
- An **actor** has a **capacity**, a dictionary of **stats** (e.g. `enchanting`, `testing`, `designing`, `sales`, `treasury`, `payroll`), and an optional **wage**. If wage is unset, the actor is **excluded** from payroll (neither wage cost nor paid-actor count). Every shipped actor sets a wage; junior staff cost `2` per month and the rest cost `3`.
...
- **Every** seed node type uses a progress system and requires an actor to apply work.
- Prerequisites:
  ...
  - `payroll`: an open payroll run whose single payment attempt has not yet been submitted
...
- **`treasury`:** holds agency money on its `money` input/output port (committed pile on the input; successful out-moves **emit** on the output for edge routing). Config `effort` (seed `1`) per **one** pending money move (in or out). Assigned actors gain progress while the pending queue is non-empty; each application dequeues one move and applies it to the committed pile (in adds; payroll outs pay whole actors that fit — see payroll).
- **`payroll`:** `money` **input** (receives routed wage payouts; does not hold stock across ticks—disbursed/consumed). Config `schedule` (`periodUnit` / `positionUnit` / `startLead` / `dueDay`; seed `month` / `day` / `0` / `10`), plus `baseEffort` and `perActorEffort` (seed `1` / `1`). On the configured start day within each period (seed: last day of the month), the sim opens one **payroll run** that snapshots every currently waged actor and wage; unwaged actors stay outside the run. Required work for the run’s single attempt = `baseEffort + perActorEffort ×` (snapshot obligation count). While the run is open and the attempt is not yet submitted, payday application (actor + progress ≥ required work) **enqueues** one pending money-out tagged with that run’s obligations when a funding edge exists from a treasury `money` port to this payroll `money` input, marks the attempt submitted, and resets progress to `0`; it does **not** debit treasury. Empty obligation / wage total `0` submits the attempt without enqueueing. Only **one** attempt is allowed per run. Actors count as paid only when treasury actually emits their full wage. On shortfall, treasury deterministically shuffles unpaid obligations, pays each **whole** wage that fits the remaining pile, leaves others unpaid, drops the rest of the move, and **does not** remove anyone immediately. After treasury processing on the due day (seed: day 10 of the following month), the transition into the next day removes only still-unpaid waged actors and their assignments, cancels any stale out-move for that run, and closes the run. A request submitted on the due day but not yet applied by treasury is late, so payroll must finish by the day **before** the due day for wages to land in time.
...
| Work / payroll | Config `effort: 10` on enchant/testing/sell; design `effort: 3`; merge `effort: 1`; treasury `effort: 1`; payroll schedule `month`/`day`/`startLead: 0`/`dueDay: 10`, `baseEffort: 1`, `perActorEffort: 1` (required work = `baseEffort + perActorEffort ×` snapshot obligation count); progress gain uses actor stats × assignment effort (`designing` / `merging` default `1`); wage obligations cover **only** roster actors with an explicit wage at run open |
| Starting stocks | Prime ports: `enchant` enchantment = genesis empty block; `treasury` money = `100`; other process ports empty; no open payroll run; pending money moves empty; block map holds genesis; node progress `0`; node cycles `0` |
| Config | Node numerics in `config/node-types/{enchant,testing,design,merge,sell,treasury,payroll}.json`; actor definitions in `config/actors/*.json`; named presets and the actor pool in `config/scenarios/`; nested time partitions in `config/time-partitions.json`; port layouts and graph construction stay in code |

### Preset `lab01`

Named JSON preset matching the original magic-agency start: `includeTesting: true`, `includeDesign: false`; roster `intern` (capacity `1.0`, wage `2`, stats `enchanting: 10`, `sales: 10`) and `boss` (capacity `1.0`, wage `3`, stats `sales: 10`, `payroll: 10`, `treasury: 10`). Preferred assignments (weight `1` each): intern → enchant, testing; boss → payroll, sell, treasury. Presets that set both optional flags are invalid.
...
Expected early behavior on `lab01`: first tick only enchant is effective among enchantment nodes (testing/sell empty); the mutated copy routes to testing and back onto enchant via the self-loop. Capacity may also split to treasury/payroll only when their prerequisites hold. Enchant mutations cost more as darkness rises (each new non-design volume unit adds `darknessDelta`). Sell deposits wait for treasury progress before the pile grows. Payroll opens near month-end and snapshots the roster's wages (lab01: `5` per month); payday needs payroll progress, then treasury progress to emit whole-actor payments before the day-10 receipt deadline. Because payroll's `money` port is consumed each tick, its committed stock returns to `0`; the durable signs of a completed payday are the treasury pile dropping and payroll's Δ column growing more negative. Console shows volume/design counts, fractional darkness/fallacy, and an abbreviated block hash.
