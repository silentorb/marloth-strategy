...
The only graph variation is whether **testing + merge** are included as a unit. When included, the `enchant→enchant` self-loop is replaced with `enchant` → `merge.primary`, and the direct `enchant→sell` edge is replaced by today’s fan-out/loop: `enchant` → `testing`; `testing` → `sell` and `testing` → `merge.secondary`; `merge` → `enchant`; money edges unchanged.
