...
### Essential graph and testing variation
...
The only graph variation is whether **testing** is included. When included, the enchant self-loop stays; `enchant→sell` is replaced by `enchant` → `testing` → `sell`, and testing also fans back onto `enchant.enchantment` (combined with the self-loop via `+`). Money edges unchanged. The merge node type stays in the catalog but is not placed in seeded graphs.

Node types (always in the catalog): `enchant` — mutate or pass-through; `testing` — remove fallacy units; `merge` — combine two enchantment inputs with the same `+` as port fan-in; `sell` — consume enchantment, emit payout; `treasury` — store money via pending moves; `payroll` — timer + payday enqueue + money input.
...
| Work / payroll | Config `effort: 10` on enchant/testing/sell; merge `effort: 1`; treasury `effort: 1`; payroll `defaultWage: 10`, `period: 5`, `effort: 1`; progress gain uses actor stats × assignment effort (`merging` default `1`); wage total covers **all** roster actors |
...
Named JSON preset matching the original magic-agency start: `includeTesting: true`; roster `intern` (capacity `1.0`, stats `enchanting: 10`, `sales: 10`) and `boss` (capacity `1.0`, stats `sales: 10`, `payroll: 10`, `treasury: 10`); wages unset. Preferred assignments (weight `1` each): intern → enchant, testing; boss → payroll, sell, treasury.
...
When generating: coin-flip testing; pick **2–4** distinct pool actors without replacement; assign preferred nodes (weight `1`) so **every graph node has at least one assignment**. Overlap (multiple actors preferred on the same node) is allowed but sparse, and more likely when actors are plentiful relative to nodes. Same `SCENARIO_SEED` yields the same graph flag, roster, and assignments.
