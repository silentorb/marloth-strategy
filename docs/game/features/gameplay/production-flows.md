...
- An **actor** has a **capacity**, a dictionary of **stats** (e.g. `enchanting`, `testing`, `designing`, `sales`, `treasury`, `payroll`), and an optional **wage**. If wage is unset, the actor is **excluded** from payroll (neither wage cost nor paid-actor count).
...
- **`payroll`:** `money` **input** (receives routed wage payouts; does not hold stock across ticks—disbursed/consumed). Config `period`, `baseEffort`, and `perActorEffort` (seed `5` / `1` / `1`). Required work per payday = `baseEffort + perActorEffort ×` (number of roster actors with an explicit wage). **Timer** increments every tick with no actor (`elapsed < period` → add 1). When `elapsed >= period`, payday is **due**. Payday application (actor + progress ≥ required work) **enqueues** a pending money-out for the wage total of paid roster actors when a funding edge exists from a treasury `money` port to this payroll `money` input, and resets progress and the timer to `0`; it does **not** debit treasury. Empty paid roster / wage total `0` resets progress and the timer without enqueueing. Shortfall is checked when treasury executes the out-move: if the pile cannot cover **all** wages, **no partial pay**—pile unchanged by that move, out-move dropped, **all actors quit**.
...
- Nested time partitions label the tick counter (seed: day / week / month). They do not change per-tick node rules. Payroll’s `period` remains a raw tick count for now.
...
| Work / payroll | Config `effort: 10` on enchant/testing/sell; design `effort: 3`; merge `effort: 1`; treasury `effort: 1`; payroll `period: 5`, `baseEffort: 1`, `perActorEffort: 1` (required work = `baseEffort + perActorEffort ×` paid-actor count); progress gain uses actor stats × assignment effort (`designing` / `merging` default `1`); wage total covers **only** roster actors with an explicit wage |
